...
% This is due to the slower relaxation of the initial plasma cloud:
% Charges deep inside of the plasma cloud are screened by outer charges,
%  and their start of drifting is delayed until the screening charges have left the cloud under the influence of the electric field. 
...
%Note that the initial distribution of the charge cloud also has an effect on the signal edges.
...
%These features are attributed to a retaining-releasing mechanism, in which charges are retained inside the initial high density charge cloud, produced by the $\alpha$-particle, 
% due to the creation of excitons, i.e.~a bound state between an electron and a hole. 
%These charges are then released with a certain temperature dependent time constant, if they have not recombined before. 
...
%The decrease in area, i.e.~total signal charge, with decreasing temperature for $T < \SI{150}{\kelvin}$ is due to enhanced recombination
% and increased release time constant from the charge cloud. 
...
%In Sec.~\ref{sec:TotSC} we discuss the dependence of the total integrated signal, i.e. the signal charge, on temperature. 
%Furthermore, for constant temperatures the total signal charge depends on the applied bias voltage which allows for charge carrier trapping time measurements,
% see Sec.~\ref{sec:VTotSC}.

%Note that at RT the decrease in pulse width with increasing bias voltage is much more pronounced than at colder temperatures. 
%For hole pulses at RT the difference in transit time $\Delta \ttr$ for $\SI{150}{\volt}$ and $\SI{400}{\volt}$,
% normalized to the transit time at $\SI{400}{\volt}$, is $r(\SI{298}{\kelvin}) = \frac{\Delta \ttr}{\ttr(400)} \approx 0.7$
% whereas at $\SI{67}{\kelvin}$ $r(\SI{67}{\kelvin}) = \frac{\Delta \ttr}{\ttr(400)} \approx 0.2$.

% The recorded current does not drop to zero after all charges have been collected but rather undershoots slightly. 
% This can be explained by the capacitance of the cable connecting the detector and the current amplifier.
% The amplitude of the undershoot correlates with the total induced signal,
%  hence at temperatures smaller than $\SI{150}{\kelvin}$ the undershoot decreases with decreasing temperature. 
...
%Its predicted temperature dependence should be consistent with an exciton binding energy of $\Ex = \SI{80}{\milli\eV}$. 
% and $t_0^+ \approx \numrange[range-phrase = -]{10}{100}\,\si{\pico\second}$.
%$\dfrac{\taurec}{\tauevap+\taurec} = \dfrac{\tau}{\tauevap}$. 
...
%This behaviour is predicted by the model under discussion. 
%The transport time at this field strength is thought to be short enough in order for the evaporation to happen almost freely, i.e.~$\tauevap = t_0 \exppEx$,
% and therefore $\tauevap$ depends exponentially on the temperature; it increases with decreasing temperature. 
%Depending on the ratio $\tau/\tauevap$, more or less excitons recombine before they thermally dissociate leading to a lower/higher measured charge,
% cf.~the second terms on the right hand side in Eqs.~(\ref{eq:Qmain}) and (\ref{eq:Qmt}). 
%Substituting $\dfrac{\tau}{\tauevap} = \dfrac{1}{1+\frac{\tauevap}{\taurec}}$ in Eq.~(\ref{eq:Qmt}) and using $\tauevap = t_0 \exppEx$  
% gives rise to a formula $f_Q(T)$, which can be fitted to the measured charge data.
The fit-function, justified in section~\ref{sec:discussion}, contains four parameters: (1) the total charge, (2) the \textit{outer} charge, cf.\ section~4, and (3) a factor $\varepsilon$ in combination with a
 Boltzmann factor % the ratio $\dfrac{\tauevap}{\taurec} = \dfrac{t_0 }{\taurec}\exppEx := \varepsilon\cdot\exppEx$
...
%Additionally, the exciton binding energy is set to its theoretical value $\Ex = \SI{80}{\meV}$. 
...
%\noindent
%and only $\varepsilon$ and $\Ea$ are \textit{a priori} completely undetermined. 
...
%Note that $t_0$ and $\taurec$ are not redundant, as their absolute values form $\tau$. 
...
%The fit result from the $Q^{t'}$-fit for the recombination lifetime, $\taurec = \SI{20(10)}{\ns}$,
% is of the same of order of magnitude as the measured one at 100\,K, 
% with the measured one within one $\sigma$ of the uncertainty of the fit result. 
]
...
%The exponential part accounts for the charge release from the inner part of the charge cloud and its respective collection. 
...
At this temperature, a certain fraction of the charge recombines. %half of the inner charges recombine, thus $\frac{1}{1 + \frac{\tauevap}{\taurec}} = \frac{1}{2}$. 
%Hence, $\tauevap(\SI{100}{\kelvin}) = \taurec \approx \SI{14(2)}{\ns}$. 
...
% \begin{figure}[!ht]
%  \centering
%  \includegraphics[width=0.75\textwidth]{figures/RD/taucomb2}
%  \caption{A comparison of the fitted shape constant $\tau$ using $\alpha$-particles on diamond (full circles) and silicon (triangles). 
% The two set-in temperatures are marked with arrows. 
% At -500\,V bias, the electric field is strong enough to break up the excitons in silicon and no tails appear (red, tip-down triangles).}
%  \label{fig:tauvsTcomb}
% \end{figure}
...
%Note that none of the parameters are free. 
...
% The influence of the integration effect of the sensor capacitance on the transit time has been evaluated.
% For this, measured voltage pulses $u_\textrm{m}(t)$ have been translated into current pulses $i_\textrm{m}(t)$, correcting for the integration effect using
% 
% \begin{equation}
%  i_m (t) = \frac{1}{A_{\textrm{amp}} R_{\text{in}}} \left[u_\textrm{m}(t) + R_{\text{in}} C_{\text{d}}\frac{\textrm{d}u_\textrm{m}(t)}{\textrm{d}t}  \right]\ .
% \label{eq:im}
% \end{equation}
% 
% \noindent The difference in transit time between input capacitance corrected current pulses and non-corrected current pulses is negligible. 
% The correction term has thus been omitted. 
...
%The error bars are omitted in order to avoid overlaying graphs. %not overlaying! small!
...
%A publication comprising a theoretical model that explains the observed electron transit times is in preparation \cite{paper:JansenEDM}. 

% Diamond, as well as silicon, is a multi-valley semiconductor, with six equivalent valleys in the conduction band. 
% The conduction band minima are positioned at $k = \frac{2\pi}{a}(\pm0.76,0,0), \frac{2\pi}{a}(0,\pm0.76,0), \frac{2\pi}{a}(0,0,\pm0.76)$.
% The surfaces of equal energy are ellipsoids, and the effective masses differ for different directions,
%  with the longitudinal mass $m_l$ along the (100) direction (two valleys) and the transverse mass $m_t$ in the plane perpendicular to the longitudinal direction (four valleys). 
% Free electrons, created by the $\alpha$-particle populate all valleys equally during the thermalisation process. 
% With an electric field applied in the (100) direction, electrons start drifting in the (100) direction,
%  but depending on the valley they are situated in they react differently to the applied field. 
% Since $m_l>m_t$, electrons are heated at a higher rate in the transverse valleys.
% This can change the population ratios,
%  as the former six nondegenerated valleys split up by an energy $\Delta$ in two valleys with degree of degeneracy equals 2 (4) for the lower (upper) state. 
% The population density ratio of the two states is $\frac{n_1}{n_0} = \frac{4}{2}\exp{(\nicefrac{-\Delta}{k_{\textrm{B}}T})}$.
% $\Delta$~is a function of the electric field strength and increases with increasing fields. 
% With increasing temperature, $\exp{(\nicefrac{-\Delta}{k_{\textrm{B}}T})}\rightarrow 1$, but how fast this happens depends on $\Delta$. 
% So for high electric field, there is no sufficient change yet in $\nicefrac{n_1}{n_0}$, and the re-population effect is not measurable. 
% For low electric field $\nicefrac{n_1}{n_0}$ changes noticeably affecting the transit time as on average more carriers are in the fast valleys, i.e.~valley with lower effective mass.
